...
if uploaded_file:
    ...
    if st.button("🧹 Clean, Validate, Analyze & Generate Dashboard"):
        with st.spinner("Processing data..."):
            ...
            with export_col1:
                excel_file = to_excel(df, cleaned_df, error_df, quarantine_df)
                st.download_button(
                    label="📊 Download Excel Report with Summary",
                    data=excel_file,
                    file_name=f"ColtraData_Report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.xlsx",
