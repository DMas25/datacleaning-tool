...
def render_internal_insights_dashboard() -> None:
    ...
    _render_breakdown_chart(chart_cols[0], "Industry mix", stats["industry_breakdown"], key="industry")
    _render_breakdown_chart(chart_cols[1], "Profession mix", stats["profession_breakdown"], key="profession")

    chart_cols2 = st.columns(2)
    _render_breakdown_chart(chart_cols2[0], "Position in organisation", stats["position_breakdown"], key="position")
    _render_breakdown_chart(chart_cols2[1], "Plan distribution", stats["plan_breakdown"], key="plan")

    st.divider()
    _render_breakdown_chart(st, "Feature usage (event counts)", stats["feature_usage_breakdown"], key="feature_usage")
    ...
    if stats["runs_last_30_days"]:
        ...
        st.plotly_chart(px.bar(trend_df, x="day", y="runs"), use_container_width=True, key="bar_runs_last_30d")
...
def _render_breakdown_chart(container, title: str, breakdown: dict, key: str = "") -> None:
    container.markdown(f"**{title}**")
    if not breakdown:
        container.caption("No data yet.")
        return
    df = pd.DataFrame({"category": list(breakdown.keys()), "count": list(breakdown.values())})
    chart_key = key or title.lower().replace(" ", "_").replace("(", "").replace(")", "").replace("/", "_")
    container.plotly_chart(px.pie(df, names="category", values="count", hole=0.4), use_container_width=True, key=f"pie_{chart_key}")
