...
class ReportBuilder:
    ...
    # ------------------------------------------------------------------ #
    # Styles — user's 4 base definitions + brand extensions               #
    # ------------------------------------------------------------------ #
    ...
    def _create_styles(self, workbook):
        ...
        primary   = self.primary    # from config["primary_colour"]
        secondary = self.secondary  # from config["secondary_colour"]
        teal  = c(BRAND["teal"])
        amber = c(BRAND["amber"])
        red   = c(BRAND["red"])
        white = c(BRAND["white"])
        ...
        self.meta_key_format = workbook.add_format({
            **base, "bold": True, "bg_color": secondary,
            "border": 1, "border_color": gray,
        })
    ...
    def _make_dashboard_png(self, raw_df, log_df):
        ...
        ax5.barh(
            ...
            edgecolor="#111827", linewidth=0.3,
        )
        ax5.invert_yaxis()
